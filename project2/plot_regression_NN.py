...
for metric in ["MSE", "R2"]:
    ...
    for i in [1,2,3,4]:
        df = pd.read_csv(f'regression_{metric}_architecture_{i}.csv')
        ...
        for j, schedule_name in enumerate(learning_rate_schedules):
            ...
            for k,activation_function in enumerate(activation_functions):
                score = df[f'{schedule_name},{activation_function}'].values
                ax.plot(lambdas,score,label=f'{k}')
            ax.legend()
            axs[0,j].set_title(schedule_name)
            axs[-1,j].set_xlabel('$\lambda$')
            axs[i-1,j].set_xscale('log')
        axs[i-1,0].set_ylabel(metric)
            
    fig.tight_layout()
    fig.savefig(f'Regression NNs {metric}.pdf')
    plt.show()